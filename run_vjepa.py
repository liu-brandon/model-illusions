...
import sys
sys.path.insert(0, "/home/users/liubr/projects/model-illusions/vjepa2")

MODEL_CACHE_DIR = "/scratch/users/liubr/model-illusions/"
...
def get_video(video_path):
    ...
    frame_idx = np.arange(0, 64, 1)
    video = vr.get_batch(frame_idx).asnumpy()
    return video


def forward_vjepa_video(model_hf, hf_transform, video_path):
...
def run_sample_inference(video_folder_path):
    ...
    model_hf = AutoModel.from_pretrained(hf_model_name, cache_dir=MODEL_CACHE_DIR)
    model_hf.cuda().eval()

    print(model_hf)

    # Build HuggingFace preprocessing transform
    hf_transform = AutoVideoProcessor.from_pretrained(hf_model_name, cache_dir=MODEL_CACHE_DIR)
    ...
    # Inference on video
    directory = Path(video_folder_path)
    for video_path in directory.iterdir():
        out_patch_features_hf = forward_vjepa_video(
            model_hf, hf_transform, video_path.as_posix()
        )
